if SERVER then
	...
	resource.AddWorkshop("310403737")
end
...
-- V: 9/5/14	
--**** AKECHI
-- Date of Akechis Update 09/02/2018 (9th Feb 2018)
-- Bugs fixed by Akechi - TTT equiptable, credits and disguiser setting preserved on prop undisguise
-- Maybe not so much a bug but the disguiser no longer sets your health to double pd_health on undisguise
-- Readded and tested convars timer won't draw if timerenabled, timertime works, and health works

-- I recommend setting your convars in your server.cfg to ensure no issues with drawing timertime removed due to it working locally but not on my server
--****
...
--CreateConVar( "pd_timerenabled", "1", ServerConvarFlags, "1/0 - Enables/disables the undisguise timer" )
CreateConVar( "pd_timertime", "25", ServerConvarFlags, "How long the timer should be running (if enabled)" )
CreateConVar( "pd_health", "50", ServerConvarFlags, "How much health the props should have" )
...
-- These two fields are most likely redundant
SWEP.TimerEnabled 		= true -- true for on false for off
SWEP.DisguiseTime 		= cvars.Number("pd_timertime") -- How long, seconds, for the player to be disguised
SWEP.DisguiseHealth 	= cvars.Number("pd_health")-- Health for the disguised prop to have. Works with about half the models
...
-- ulx luarun for k,v in pairs(player.GetAll()) do if v:Nick() == 'Shisno' then local wep = v:GetWeapon(); print(wep:GetNWBool("PD_WepDisguised"))
...
function SWEP:PropDisguise()
	...
	if SERVER then
		-- Save health
		-- Stop us from dropping
		self.AllowDrop = false
		self.SavedHP = ply:Health()
		-- Undisguise after the time limit
		if self.TimerEnabled then
			timer.Create(ply:SteamID().."_DisguiseTime", cvars.Number("pd_timertime"), 1, function() 
				self:SetNWBool("PD_TimeOut", true)
				self:SetNextPrimaryFire(CurTime()+self.Primary.Delay + 5) -- Small delay after timer going out
				self:PropUnDisguise() 
			end)
		end
		ply:SetNWFloat("PD_TimeLeft", CurTime() + cvars.Number("pd_timertime")) -- Clientside timer
		...
		prop.fakehp = cvars.Number("pd_health") -- Using our own health value
		prop.hp_constant = cvars.Number("pd_health")
		ply:SetHealth(cvars.Number("pd_health")) -- This is the prop's health but displayed as their own
...
function SWEP:PropUnDisguise()
	...
	if IsValid(self.Prop) and IsValid(self.Owner) and self:GetNWBool("PD_WepDisguised") then
		...
		ply:SetHealth( self.SavedHP) -- health set to whatever we had it as before
		-- Props health is set by convar using the stored health value from before we disguised prevents players from healing
...
function SWEP:DrawHUD()
	...
	--local convarTimerOn = cvars.Bool("pd_timerenabled")
	local convarTimerOn = self.TimerEnabled
	
	--print(disguised, propped)
	if disguised and propped and convarTimerOn then
		local w = ScrW()
		local h = ScrH()
		...
		timeleft = math.Clamp(timeleft, 0, cvars.Number("pd_timertime")) -- Clamp to prevent negatives
		
		local Segments = width / cvars.Number("pd_timertime") -- Divide the width into the timer 
...
local function DamageHandler( ent, dmginfo ) -- Entity Take Damage
	...
	if ent.IsADisguise and SERVER and IsValid(ent.TiedPly) then
		...
		if ent.fakehp <= 0 then 
			--self.AllowDrop = true
			local selfsave = ply:GetWeapon("weapon_ttt_prophide")
			selfsave.AllowDrop = true
			ply:SetNWBool("PD_Disguised", false)
			selfsave:SetNWBool("PD_WepDisguised", false)
